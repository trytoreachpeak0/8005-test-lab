...
`lab watch status --all-machines` 的形状定于 #33（是一个开关，不是新子命令）；**#34 定了它要有**，
判据是那条不出声的路径 —— 上一个会话的控制端异常终止、探针带着未过期的 Grant 留在车上，新会话读
本地 `observations/` 看到「已结束」，起一次 run 就会被那个探针的 Repair 改掉现场。缺省去机器上读、
不读本地记录；`tier: production` 的机器包含在内不需点名（纯只读，不绑定不执行动词）；一台机器
不可达时那台的答案是 `unknown`，**只要有一台 `unknown`，整条命令的结论就不许被读成「没人在看着」**。
细节见 `91-agent-briefing.md` 第 5、6 节。
...
2. 每个子命令的**结局取值全集** —— `run` 是 `PASS`/`FAIL`/`ERROR`，`machine check` 含
   ...
   手写的那一小块，跟着各子命令的实现走。
3. 退出码表。
4. **`docs`：`docs/spec/` 的现场派生索引**（每份文件的文件名、一级标题、头行 `<!-- 来源：#N -->`
   里的票号），#34 加。理由与前三样同源 —— 一个新会话要知道「该读哪一份」，而一份写在文档里的
   规格清单就是那份会被忘记更新的手写清单。**它同时是 `docs/spec/README.md` 分片表的守卫**：
   两份读回来比对，对不上即失败，读不回来也算失败（#24 「不腐烂」第 2 条）。那张表 2026-09-09
   漏过一行（`96-ci.md` 落地了而表里没有）。

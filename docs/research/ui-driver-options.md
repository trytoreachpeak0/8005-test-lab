...
> **这是 2026-09-04 的快照，不回改。**当时没有在任何机器上跑过这三样东西
> （见下一段）。[#7](https://github.com/trytoreachpeak0/8005-test-lab/issues/7) 于 2026-09-07 在
> `win11-01` 上把最后一节列的七个点实测掉了，其中一条直接推翻了本文：**`winapp ui` 不是自包含
> 单文件** —— `winappcli-x64.zip` 解开是三个文件，投递负载是 `winapp.exe`（31.5 MB）加
> `libSkiaSharp.dll`（9.4 MB）共约 41 MB，`winapp.pdb`（147 MB）丢掉。实测还发现一条本文没有
> 预料到的行为：锁屏下 `--capture-screen` **返回退出码 0 加一张锁屏壁纸**，静默给错答案。
> 本文的推荐结论（选 `winapp ui`、FlaUI 作备选）没有被推翻。以票的决议为准，本文只作为当时依据的
> 记录。
